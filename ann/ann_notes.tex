...
\newpage
  ...
  \section{Neurons and Neural Networks}
    ...
    \subsection{Neuron}
      ...
      for example, a single pixel's luminosity level in a photo, scaled to the
...
\newpage
  ...
  \section{Learning, gradient descent, backpropagation}
    ...
    In some specific cases (e.g. Lipschitz-continuous convex functions),
    guaranteed convergence can be proven and its speed can be calculated, but
    in practice, the error function rarely has such nice properties, so
    experimentation tends to be more useful than attempting to produce formal
    theoretical proofs.
...
\newpage
    ...
    \subsection{Basic backpropagation algorithm}
      ...
      is close to an optimum, the chance of overshooting can be minimized.
...
\newpage
    ...
    \subsection{Training and testing (validating)}
      ...
      two subsets: the training data and the validation data (also known as
      test data). Usually, the training data contains more elements than the
      validation data, e.g. with a $2:1$ or a $3:1$ ratio.
...
\newpage
  ...
  \section{Advanced topics}
    ...
    \subsection{Preprocessing}
      ...
      Depending on several factors (e.g. the problem at hand, the choice of
      activation functions, the format and the interpretation of the input and
      output data, numerical considerations, etc.), various kinds of
      preprocessing can significantly improve the network's performance.

      \subsubsection{Statistical methods}

        For $x \in \mathbb{R}$, when $x$ is sufficiently far from $0$, most of
        the activation functions (\ref{actfuncs}) behave linearly in the sense
        that for $0 < c \in \mathbb{R}$, $f(c \cdot x) = c \cdot f(x)$. This
        implies that if the entire input of the network is scaled by a factor of
        $c$, then its output will also be scaled by the same amount. If the
        expected output remains fixed, then this scaling effect will be
        compensated by the training process by introducing a scaling factor of
        $\frac{1}{c}$ into the weights and biases in the network. It will also
        be reflected in the gradient, thus, it will affect the optimal choice
        of the learning rate as well. If the network has to do too much of this
        compensation, then it will be more suspectible to issues like the
        "vanishing gradient problem" where the gradient becomes so small for
        most inputs that basically no learning occurs. This, together with
        floating point arithmetics considerations in practical implementations,
        suggests that in most cases, it can be beneficial to bring the network's
        input closer to the $[0, 1]$ or the $[-1, 1]$ intervals.

        \paragraph{Normalization}

          Let $u, l \in \mathbb{R}$ (typically, $l = 1$ and $u \in \{-1, 0\}$),
          and let:

          \begin{align*}
            x_{\min} & = \min\left\{
              x_i : (\mathbf{x}, \mathbf{y}) \in trainers,
                    \ i \in \{1, 2, \dots, n_0\}
            \right\} \\
            x_{\max} & = \max\left\{
              x_i : (\mathbf{x}, \mathbf{y}) \in trainers,
          ...
          Note that only the $trainers$ set is used from algorithm
          \ref{alglearnval} for determining these values, since we don't want
          any information to be leaked into the training process from the
          validation set.

          Before the network is evaluated for a given $\mathbf{x}$ input, it is
          first transformed with the following formula
          (for $j \in \{1, 2, \dots, n_0\}$):

          \begin{align*}
            \hat{x}_j
              = \frac{x_j - x_{\min}}{x_{\max} - x_{\min}} \cdot (u - l) + l
              \text{ for } j \in \{ 1, 2, \dots, n_0 \}
          \end{align*}

          Note: if the input is composed from heterogeneous features from
          different ranges, then it might be a good idea to normalize them
          separately or by feature groups.
          For $j \in \{1, 2, \dots, n_0\}$ and $u_j, l_j \in \mathbb{R}$, let

          \begin{align*}
            x_{j \min} & = \min\left\{
              x_j : (\mathbf{x}, \mathbf{y}) \in trainers
            \right\} \\
            x_{j \max} & = \max\left\{
              x_j : (\mathbf{x}, \mathbf{y}) \in trainers
            \right\} \\
          ...
          \begin{align*}
            \hat{x}_j
              = \frac{x_j - x_{j \min}}{x_{j \max} - x_{j \min}}
                \cdot (u_j - l_j) + l_j
          \end{align*}

        \paragraph{Standardization}

          For the analysis of statistical data, the exact value of its samples,
          its mean, and its deviation from the mean are often less important
          than the shape of its distribution. Standardization is a technique
          which brings statistical data into a single well defined location on
          the number line so that its distribution can be analyzed in well
          defined circumstances.

          Standardization requires the mean and the standard deviation of the
          theoretical distribution to be known, but this is rarely the case in
          practice, so their estimated values are used instead. Let
          $N \in \mathbb{N}^+$ denote the number of examples in the $training$
          set:

          \begin{align*}
            \mu
              & = \frac{1}{N \cdot n_0}
                  \cdot \sum_{\mathbf{x} \in training} \sum_{i=0}^{n_0} x_i \\
            \sigma
              & = \sqrt{
                    \frac{1}{N \cdot n_0 - 1}
                    \cdot \sum_{\mathbf{x} \in training}
                      \sum_{i=0}^{n_0} \left( x_i - \mu \right)^2
                  }
          \end{align*}

          Then, before the network is evaluated for a given $\mathbf{x}$ input,
          $\mathbf{x}$ is first transformed with the following formula
          (for $j \in \{1, 2, \dots, n_0\}$):
          ...
          Again, if the input is composed of heterogeneous features with
          different ranges and distributions, then standardization is also
          better performed separately for the different features (or feature
          groups). For $j \in \{1, 2, \dots, n_0\}$:

          \begin{align*}
            \mu_j
              & = \frac{1}{N}
                  \cdot \sum_{\mathbf{x} \in training} x_j \\
            \sigma_j
              & = \sqrt{
                    \frac{1}{N - 1}
                    \cdot \sum_{\mathbf{x} \in training}
