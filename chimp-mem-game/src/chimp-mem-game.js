...
function is_mute()
{
    return mute_node.checked || audio_ctx === null;
}
...
function init_audio()
{
    ...
    win_lfo_freq_shaper_curve = new Float32Array(
        [0.0, win_frequency]
    );
    ...
    wrong_frequency_2 = (
        wrong_frequency_1 * Math.pow(2.0, -19.0 / 12.0)
    );
    wrong_amp_curve = new Float32Array([-1.0, 1.0]);
    wrong_freq_curve = new Float32Array(
        [0.0, wrong_frequency_1 * 2.5]
    );
}
